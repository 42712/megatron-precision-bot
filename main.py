"""
Megatron Precision Bot v2.1 — MODO REAL
...
def home():
    return jsonify({
        ...
        'status': 'online',
        'modo': '🔴 REAL (Binance)' if not MODO_TESTE else '🔵 SIMULAÇÃO',
        'pares': PARES,
        'configuracao': {
...
def bot_loop():
    print("🤖 Bot TURBINADO v2.1 iniciado - MODO REAL!")
    contador = 0
    while True:
        try:
            saldo = trader.get_saldo()
...
if __name__ == "__main__":
    ...
    🔴 ATENÇÃO: O BOT ESTÁ EM MODO REAL!
    ⚠️  Operações serão executadas na sua conta Binance!
    """)
